...
from src.ui import odstranit_ukol_logika
...
def test_delete_task_positive():
    ...
    try:
        ...
        assert result == "Úkol byl odstraněn", f"Očekávalo se 'Úkol byl odstraněn', ale funkce vrátila: {result}"
...
def test_delete_task_negative():
    """Negativní test na odstranění neexistujícího úkolu."""
    gen = pripojeni_db()
    conn = next(gen)
    try:
        id_ukolu = 999
        ...
        result = odstranit_ukol_logika(conn, id_ukolu, "ano")
        assert result == "Úkol neexistuje", f"Funkce měla vrátit 'Úkol neexistuje', ale vrátila {result}"
